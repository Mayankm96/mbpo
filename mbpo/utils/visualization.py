...
def visualize_policy(real_env, fake_env, policy, writer, timestep, max_steps=100, focus=None, label='model_vis',
                     img_dim=128):
    if hasattr(real_env, 'reset_vis'):
        init_obs = real_env.reset_vis()
    else:
    ...
    while not (term_r and term_f) and i <= max_steps:
        ...
        if not term_r:
            ...
            images_r.append(real_env.render(mode='rgb_array'))
    ...
    record_trajectories(writer, label, timestep, images_r)
